        ...
        {#if activeTab === 'login'}
            <!-- ログインフォーム -->
             <form method="POST" action="?/login" class="flex flex-col gap-6">
                ...
                {/if}
                <input class="input validator w-full" type="email" name="email" required placeholder="mail@site.com" />
                {#if form?.errors?.password}
                  <div class="text-red-500 text-sm mt-1">{form.errors.password[0]}</div>
                {/if}
                <input class="input validator w-full" type="password" name="password" required placeholder="パスワード" />
